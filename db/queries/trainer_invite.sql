...
WITH ranked_link AS (
  ...
  ORDER BY
    tc.client_user_id,
    CASE
      WHEN sqlc.narg('prefer_trainer_id')::uuid IS NOT NULL
        AND tc.trainer_id = sqlc.narg('prefer_trainer_id')::uuid
      THEN 0
      ELSE 1
    END,
    tc.created_at DESC
...
ORDER BY
  CASE WHEN sqlc.arg('sort_by') = 'display_name' AND sqlc.arg('sort_order') = 'asc' THEN u.display_name END ASC NULLS LAST,
  CASE WHEN sqlc.arg('sort_by') = 'display_name' AND sqlc.arg('sort_order') = 'desc' THEN u.display_name END DESC NULLS LAST,
  CASE WHEN sqlc.arg('sort_by') = 'linked_at' AND sqlc.arg('sort_order') = 'asc' THEN rl.created_at END ASC NULLS LAST,
  CASE WHEN sqlc.arg('sort_by') = 'linked_at' AND sqlc.arg('sort_order') = 'desc' THEN rl.created_at END DESC NULLS LAST,
