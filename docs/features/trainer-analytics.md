...
- **Лента** (`/completions`) — пагинация + `from`/`to` (RFC3339 или `YYYY-MM-DD`, `to` эксклюзивно), сортировка `completed_at` desc; `is_current_cycle` — принадлежность текущему назначению.
