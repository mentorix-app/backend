...
- **Пустой список = общий блок.** `client_user_ids: []` — блок виден всем клиентам,
  назначенным на программу; непустой список — только им.
...
- **Инвариант дня.** У дня всегда должен остаться хотя бы один общий блок. Проверка —
  ...
  **и** по каждой опубликованной версии с активным назначением, потому что клиенты
  сидят на этих версиях прямо сейчас, без publish, — и снова для рабочей копии при
  `publish`.
