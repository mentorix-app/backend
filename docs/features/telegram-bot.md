...
- Меню: reply keyboard (`Программа`, `Тренеры`, `Помощь`) + inline для выбора тренера и навигации по программе + «Статистика» при заданном `CLIENT_ANALYTICS_PAGE_URL`.
- **Активный тренер:** Redis (`active_trainer` key — см. [redis-naming.md](../../.claude/rules/redis-naming.md)).
  ...
  - Нет активного (2+ тренеров, ключ в Redis отсутствует/устарел): «Тренеры» всё равно показывает список + inline; «Программа» — просьба выбрать тренера в «Тренеры».
...
- **Видимость блоков:** клиент видит только блоки, доступные ему — [program-block-visibility.md](program-block-visibility.md). `trainerclient.clientProgramView` вызывает `program.GetVersionDetailForClient`, которая дозагружает версию и убирает чужие блоки; `day_snapshot` при отметке тренировки строится из уже отфильтрованного дня, так что скрытые блоки не попадают и в журнал.
...
| Статистика | inline URL-кнопка на страницу статистики у активного тренера, ссылка живёт 30 минут — [client-analytics.md](client-analytics.md) |
| Помощь | справка |
...
| `CLIENT_ANALYTICS_PAGE_URL` | нет | кнопка «Статистика»; пусто — кнопки нет |
