...
- **Видимость блоков:** клиент видит только блоки, доступные ему (`program_block_clients` — [program-blocks.md](program-blocks.md)). `trainerclient.clientProgramView` вызывает `program.GetVersionDetailForClient`, которая дозагружает версию и убирает чужие блоки; `day_snapshot` при отметке тренировки строится из уже отфильтрованного дня, так что скрытые блоки не попадают и в журнал.
