...
Формат «Сегодня» / «Программа» / «Тренеры»: иконки в заголовке (`📋`/`📅`/`👤`, `👤`, `💪`, `📆`), упражнения `🏋 название - 3х8` с инструкцией курсивом; групповые блоки — иконка типа (`🧩 Комплекс:`, `🔗 Суперсет:` и т.д.), инструкция блока, упражнения `А.` / `Б.`; отдых — `😴`. «Тренеры»: карточка `👤 Тренер:` + `💪 Программа:`; при нескольких — `✓` у активного, inline-выбор.
